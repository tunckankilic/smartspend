...
void main() {
  ...
  group('exportExpenses — success', () {
    ...
    test(
      'should invoke the pdf function name when format is ExportFormat.pdf',
      () async {
        when(
          () => functions.invoke(
            any(),
    ...
    test(
      'should invoke the csv function name when format is ExportFormat.csv',
      () async {
        when(
          () => functions.invoke(
            any(),
    ...
    test(
      'should pass from_date and to_date as yyyy-MM-dd query params',
      () async {
        Map<String, dynamic>? captured;
    ...
    test('should use HttpMethod.get when invoking the function', () async {
      ...
      when(
        ...
        capturedMethod = inv.namedArguments[#method] as HttpMethod?;
        return _goodResponse();
      });
  ...
  group('exportExpenses — HTTP errors', () {
    test('should throw ServerException when status is 400', () async {
      when(
        ...
        (_) async => const FunctionResponse(status: 400, data: null),
      );
    ...
    test('should throw ServerException when status is 500', () async {
      when(
        ...
        (_) async => const FunctionResponse(status: 500, data: null),
      );
  ...
  group('exportExpenses — bad payload shapes', () {
    test('should throw ServerException when data is not a Map', () async {
      when(
        ...
        (_) async => const FunctionResponse(status: 200, data: 'plain string'),
      );
    ...
    test(
      'should throw ServerException when "data" envelope key is missing',
      () async {
        when(
          () => functions.invoke(
            any(),
    ...
    test(
      'should throw ServerException when "data" envelope is not a Map',
      () async {
        when(
          () => functions.invoke(
            any(),
    ...
    test(
      'should throw ServerException when "row_count" is a String, not int',
      () async {
        when(
          () => functions.invoke(
            any(),
  ...
  group('exportExpenses — SDK exceptions', () {
    test(
      'should throw ServerException when FunctionException is thrown',
      () async {
        when(
          () => functions.invoke(
            any(),
    ...
    test(
      'should throw ServerException when a generic Exception is thrown',
      () async {
        when(
          () => functions.invoke(
            any(),
